...
def demo01_KNN_classification():
    ...
    # 交叉验证和网格搜索，API是找到最好的超参数组合后(这里只有一个超参数n_neighbors），再用这个超参数再全量训练集上训练。
    estimator = GridSearchCV(estimator,param_grid={"n_neighbors":[2,3,4,5,6,7]},cv=3)
    estimator.fit(x_train,y_train)
    print("最好的模型评分：",estimator.best_score_)
